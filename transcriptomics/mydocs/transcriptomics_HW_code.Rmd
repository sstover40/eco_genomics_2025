...
would need statistical analysis to determine if this is significant or not. Permutations? 
...
# RAN INTO ISSUES WITH THE SCALES
# Combine into 3-panel figure
#combined_plot <- (plotG1v4 | plotG2v4 | plotG3v4) + plot_layout(guides = "collect")
#combined_plot
```
Can we output the enrichment results from TopGO and try REViGO 
```{r}
# Select only the columns REVIGO needs
revigo_inputG1v4 <- filtered_GO_resultsG1v4[, c("GO.ID", "classicFisher")] #this table will be used to filter
revigo_inputG2v4 <- filtered_GO_resultsG2v4[, c("GO.ID", "classicFisher")]
revigo_inputG3v4 <- filtered_GO_resultsG3v4[, c("GO.ID", "classicFisher")]
...
Making an Upset Plot with the comparisons between generations
```{r}

```

Testing DEG between control generations: 
PCA plot to visualize gene expression variation among treatment Generations
```{r}
####################################################
...
ggplot(pcaData_filt, aes(PC1, PC2)) +
  ...
  theme(legend.title = element_blank())
```
looks like most of the variance (PC1) is between G1, G2 and the other generations. PC2 seems to vary between replicates but there may be a slight pattern of G1 and G2 being more spread out while G3 and G4 cluster....
...
```{r}
#comparison
resC_G1v2 <- results(dds, contrast=c("group","controlG1","controlG2"), alpha = 0.05) #alpha = significance value 

summary(resC_G1v2)

#differentiate up reg and down reg DEGs
up_G1v2 <- sum(resC_G1v2$padj < 0.05 & resC_G1v2$log2FoldChange > 0, na.rm=TRUE)
down_G1v2 <- sum(resC_G1v2$padj < 0.05 & resC_G1v2$log2FoldChange < 0, na.rm=TRUE)
```
2. G1 and G3
```{r}
resC_G1v3 <- results(dds, contrast=c("group","controlG1","controlG3"), alpha = 0.05) #alpha = significance value 

summary(resC_G1v3)

#differentiate up reg and down reg DEGs
up_G1v3 <- sum(resC_G1v3$padj < 0.05 & resC_G1v3$log2FoldChange > 0, na.rm=TRUE)
down_G1v3 <- sum(resC_G1v3$padj < 0.05 & resC_G1v3$log2FoldChange < 0, na.rm=TRUE)
```
3. G1 and G4
```{r}
resC_G1v4 <- results(dds, contrast=c("group","controlG1","controlG4"), alpha = 0.05) #alpha = significance value 

summary(resC_G1v4)

#differentiate up reg and down reg DEGs
up_G1v4 <- sum(resC_G1v4$padj < 0.05 & resC_G1v4$log2FoldChange > 0, na.rm=TRUE)
down_G1v4 <- sum(resC_G1v4$padj < 0.05 & resC_G1v4$log2FoldChange < 0, na.rm=TRUE)
```
4. G2 and G3
```{r}
resC_G2v3 <- results(dds, contrast=c("group","controlG2","controlG3"), alpha = 0.05) #alpha = significance value 

summary(resC_G2v3)

#differentiate up reg and down reg DEGs
up_G2v3 <- sum(resC_G2v3$padj < 0.05 & resC_G2v3$log2FoldChange > 0, na.rm=TRUE)
down_G2v3 <- sum(resC_G2v3$padj < 0.05 & resC_G2v3$log2FoldChange < 0, na.rm=TRUE)
```
5. G2 and G4
```{r}
resC_G2v4 <- results(dds, contrast=c("group","controlG2","controlG4"), alpha = 0.05) #alpha = significance value 

summary(resC_G2v4)

#differentiate up reg and down reg DEGs
up_G2v4 <- sum(resC_G2v4$padj < 0.05 & resC_G2v4$log2FoldChange > 0, na.rm=TRUE)
down_G2v4 <- sum(resC_G2v4$padj < 0.05 & resC_G2v4$log2FoldChange < 0, na.rm=TRUE)
```
6. G3 and G4 
```{r}
resC_G3v4 <- results(dds, contrast=c("group","controlG3","controlG4"), alpha = 0.05) #alpha = significance value 

summary(resC_G3v4)

#differentiate up reg and down reg DEGs
up_G3v4 <- sum(resC_G3v4$padj < 0.05 & resC_G3v4$log2FoldChange > 0, na.rm=TRUE)
down_G3v4 <- sum(resC_G3v4$padj < 0.05 & resC_G3v4$log2FoldChange < 0, na.rm=TRUE)
```
...
Question: How does gene expression change across generations for the genes that are differentially expressed from G1 to G2
```{r}
sig_genes_G1v2 <- rownames(resC_G1v2[which(resC_G1v2$padj < 0.05 & !is.na(resC_G1v2$padj)), ])

# Combine into a data frame
lfc_mat <- data.frame(
  Gen1v2 = resC_G1v2[sig_genes_G1v2, "log2FoldChange"],
  Gen1v3 = resC_G1v3[sig_genes_G1v2, "log2FoldChange"],
  Gen1v4 = resC_G1v4[sig_genes_G1v2, "log2FoldChange"]
#  Gen4 = resG4_CvT[sig_genes_G1, "log2FoldChange"]
)
