...
ggVennDiagram(venn_data, label_alpha = 0.5) +
  ...
  theme(
    ...
    venn.setLabel = element_text(size = 30, color = "black", face = "bold"),
...
# 1. Compute scaffold offsets from SNP data
scaffold_offsets <- allsnp_manhattan %>%
  group_by(Chr) %>%
  summarise(max_bp = max(position), .groups = "drop") %>%
  mutate(offset = lag(cumsum(max_bp), default = 0))

# 2. Prepare selection windows from theta2_filt
selection_windows <- theta2_filt %>%
  # Make sure Chr is a factor with correct levels
  mutate(
    Chr = factor(Chr, levels = levels(allsnp_manhattan$Chr)),
    win_start = WinCenter - 50000,
    win_end = WinCenter + 50000
  ) %>%
  left_join(scaffold_offsets, by = "Chr") %>%
  mutate(
    genomic_start = win_start + offset,
    genomic_end = win_end + offset
  )

# 3. Plotting
ggplot() +
  # Draw selection regions as transparent violet rectangles
