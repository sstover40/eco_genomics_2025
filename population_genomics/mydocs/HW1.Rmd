...
theta2$sig_status <- ifelse((theta2$Tajima<-1.5) & (theta2$tPsite<cutoff), "Both",
                       ifelse(theta2$Tajima<-1.5, "Tajima",
                       ifelse(theta2$tPsite<0.001, "Pi", "None")))
...
venn <- ggVennDiagram(venn_data, label_alpha = 0.5) +
  scale_fill_distiller(palette = "GnBu") +
  theme(
    legend.position = "right",
    venn.setLabel = element_text(size = 10, color = "black"),
...
SNP_Manhattan <- ggplot(allsnp_manhattan, aes(x = genomic_pos, y = logp, color = SNP_status)) +
